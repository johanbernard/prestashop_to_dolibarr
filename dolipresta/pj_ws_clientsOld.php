...
$thirdparty_fields= array(
	...
	'addressesClient' => array('name'=>'addressesClient','type'=>'tns:AddressesClientArray') //ajout de l'adresse
);
		...
		*/
		$server->wsdl->addComplexType(
			'AddressesClientArray',
			'complexType',
			'array',
			'sequence',
			'',
			array(
				'addresseClient' => array(
				'name' => 'addresseClient',
				'type' => 'tns:addresseClient',
				'minOccurs' => '0',
				'maxOccurs' => 'unbounded'
				)
			)
...
function createThirdParty($authentication,$thirdparty)
{
    ...
    if (! $error)
    {
        ...
        if (! $error)
        {
			...
			*/
			for ($x = 0; $thirdparty['addressesClient']['addresseClient'][$x]['zip'] != ""; $x++) {
				
				$nom 			= $thirdparty['addressesClient']['addresseClient'][$x]['nom'];
				$prenom 		= $thirdparty['addressesClient']['addresseClient'][$x]['prenom'];						
				$adresse 		= $thirdparty['addressesClient']['addresseClient'][$x]['adresse']; 
				$codePostal 	= $thirdparty['addressesClient']['addresseClient'][$x]['zip'];
				$ville 			= $thirdparty['addressesClient']['addresseClient'][$x]['town'];
				$countryCodeISO = $thirdparty['addressesClient']['addresseClient'][$x]['country_code'];
				$telephone 		= $thirdparty['addressesClient']['addresseClient'][$x]['phone'];
				...
				$contact->zip = $codePostal;
				$contact->town = $ville;
				$contact->email = "";
				$contact->phone_pro = $telephone;
				$contact->phone_mobile = $telephone;
				$contact->fax = "";
				$contact->statut = 1;
				$contact->country_id = getCountry($countryCodeISO,3);
				$contact->country_code = $countryCodeISO;
				$contact->import_key = $thirdparty['addressesClient']['addresseClient'][$x]['import_key'];
		...
		if ((! $error) && ($objectfound))
		{
			...
			*/
			...
			for ($x = 0; $thirdparty['addressesClient']['addresseClient'][$x]['zip'] != ""; $x++) {
				...
				//fetch sur import_key pour vérifier l'existance
				$fetchid = getImportedSocpeople($contact->import_key);
				
				if ($fetchid < 0){ // error
					$error++;
					$errorcode='KO';
					$errorlabel=$contact->error;
				}
				else {
					if ($fetchid == ""){ // non trouvé => création
						$resultc =$contact->create($fuser);
					}
					else {	//id trouvé => update
						$resultc = $contact->update($fetchid, $fuser);
					}				
