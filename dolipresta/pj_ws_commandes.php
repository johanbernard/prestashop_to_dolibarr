	...
	if (! $error)
	{
		...
		if ($result < 0)
		{
			...
			if ($order['status'] == '2') { //Validation
				...
				dol_syslog('createorder update status 2!');
				$result=$newobject->valid($fuser);
				dol_syslog('createorder update status 3!');
				$result=$newobject->setStatut(2);
				...
				dol_syslog('createorder update status 2!');
				$result=$newobject->valid($fuser);
				dol_syslog('createorder update status 6!');
				$result=$newobject->cancel();
				dol_syslog('result cancel order : '.print_r($result, true));
			}
			dol_syslog('tentative de up le statut finie : '.$order['status']);
			if ($result < 0) {
				$db->rollback();
				$error++;
				$errorcode='KO';
		...
		if (!$error) {
			...
			if ($order['id_address_delivery']) {
				dol_syslog('createorder adresse delivery');
				$resultg = getImportedSocpeople($order['id_address_delivery']);
				if ($resultg >=0) {
					//link adresse delivery
					dol_syslog('createorder adresse delivery link');
			...
			if ($order['id_address_invoice']) {
				...
				if ($resultg >=0) {
					...
					dol_syslog('createorder adresse invoice link');
...
function updateOrderStatus($authentication,$id='',$status='1')
{
	...
	if (! $error)
	{
		...
		if ($fuser->rights->commande->lire)
		{
			...
			if ($result > 0)
			{
				dol_syslog('updateorderstatus check A  status = '.$status.' & order->statut = '.$order->statut);
				if ($status == 1) { // Brouillon
					if ($order->statut != 0){ //Vérif si le statut n'est pas deja celui qu'on cherche
						dol_syslog('updateorderstatus check 1  status = '.$status);
						$result=$order->set_draft($fuser);
						dol_syslog('result draft order : '.print_r($result, true));
					}
				} elseif ($status == 2) { //Validation
					if ($order->statut != 1){ //Vérif si le statut n'est pas deja celui qu'on cherche
						dol_syslog('updateorderstatus check 2  status = '.$status);
						$result=$order->valid($fuser);
						dol_syslog('result valid order : '.print_r($result, true));
					}
				} elseif($status == 3){ //En cours de traitement
					if ($order->statut == 1){ //statut brouillon => il faut valider
						$result=$order->valid($fuser);
						dol_syslog('result valid order : '.print_r($result, true));
					}
					if ($order->statut != 2){ //Vérif si le statut n'est pas deja celui qu'on cherche
						dol_syslog('updateorderstatus check 3  status = '.$status);
					...
					if ($order->statut != 3){ //Vérif si le statut n'est pas deja celui qu'on cherche
						dol_syslog('updateorderstatus check 4  status = '.$status);
					...
					if ($order->statut != 3) { //verif si la commande est bien clôturée
						$result=$order->cloture($fuser);
						dol_syslog('result cloture order : '.print_r($result, true));
					}
					if (!$order->billed) {
					...
					if ($order->statut != -1){ //Vérif si le statut n'est pas deja celui qu'on cherche
						dol_syslog('updateorderstatus check 6  status = '.$status);
						$result=$order->cancel();
						dol_syslog('result cancel order : '.print_r($result, true));
					}
				} else {
					dol_syslog('pdateorderstatus check 7');
				}
				dol_syslog('updateorderstatus check B');
				if ($result < 0) {
					$db->rollback();
					$error++;
					$errorcode='KO';
