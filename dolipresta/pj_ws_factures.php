...
function createInvoice($authentication,$invoice)
{
    ...
    if (! $error)
    {
		...
		if (!$error) {
			//delivery address
			if ($invoice['id_address_delivery']) {
				$resultg = getImportedSocpeople($invoice['id_address_delivery']);
				if ($resultg >=0) {
					//link address delivery
					$newobject->add_contact($resultg, 61, $source='external',$notrigger=0);
				}
			}
			
			//invoice address
			if ($invoice['id_address_invoice']) {
				$resultg = getImportedSocpeople($invoice['id_address_invoice']);
				if ($resultg >=0) {
					//link address invoice
					$newobject->add_contact($resultg, 60, $source='external',$notrigger=0);
				}
			}
		}
